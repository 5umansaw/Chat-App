...
io.on('connection' , (socket) =>{
    ...
    socket.on('join' , ({username , room} , callback) =>{
        ...
        socket.emit('msgSend', generateMessages('Admin' , 'Welcome!'))
        socket.broadcast.to(user.room).emit('msgSend' , generateMessages('Admin' , `${user.username} has joined`))
    ...
    socket.on('disconnect' , () => {
        ...
        if(user){
           io.to(user.room).emit('msgSend' , generateMessages('Admin' , `${user.username} has left!`))
           io.to(user.room).emit('roomData', {
              room: user.room,
              users: getUsersInRoom(user.room)
           })
